...
class Game:
    ...
    def display_level_number(self, level):
        font = pygame.font.Font(None, 50)  # You can change the font size or type as needed
        level_text = font.render(f"Level {level + 1}", True, (255, 255, 255))  # Create the level display text
        self.display.fill((0, 0, 0))  # Clear the display with a black background
        self.display.blit(level_text, (self.display.get_width() // 2 - level_text.get_width() // 2,
                                       self.display.get_height() // 2 - level_text.get_height() // 2))
        self.screen.blit(pygame.transform.scale(self.display, self.screen.get_size()), (0, 0))
        pygame.display.update()
        pygame.time.wait(2000)  # Wait for 2 seconds to show the level number
    ...
    def load_level(self, map_id):
        ...
        self.display_level_number(map_id)
